...
def ffc_sync():
    ...
    volume, or how many different people are using this site's shared
    instance - the cache is keyed by scoring/team-size only, not by who's
    asking, so everyone hitting Sync draws from the same once-a-day
    cache rather than each triggering their own fetch."""

...
class DraftIQEngine:
    ...
    # Positions capped at a single roster copy in a standard lineup - once
    # the user has one, a second is never useful, so these get excluded
    # from recommendations entirely (rather than merely deprioritized)
    # once drafted.
    SINGLE_COPY_RECOMMENDATION_POSITIONS = {"DEF", "K"}

    def _positions_excluded_from_recommendations(self):
        """Positions to drop from recommendation/decision candidate pools
        because the user has already drafted their one allotted copy."""
        counts = self.get_position_counts()
        return {
            pos for pos in self.SINGLE_COPY_RECOMMENDATION_POSITIONS
    ...
    def calculate_position_urgency(self, position, tier_table=None, position_counts=None,
        ...
        # Once a team already has its required QB(s), the round-based
        # "you should be prioritizing this position now" bumps below no
        # longer apply - unlike base_need above, those weren't gated by
        # current_count vs required at all, so a team that already had
        # its starter kept getting the same "Middle-round priority" push
        # as a team with none. Scarcity/run signals further down are
        # left alone here (not skipped) since they're a genuinely
        # different, still-relevant question ("is this guy a rare
        # value"), just lightly dampened at the end - a real standout
        # should still be able to surface, just not as readily as when
        # the slot is actually open.
        qb_need_met = position == "QB" and required > 0 and current_count >= required
        ...
        if round_number <= 3:
            ...
            if position in ["RB", "WR"]:
                urgency += 10
            elif position in ["TE", "QB"] and not qb_need_met:
                urgency += 8
                reasons.append("Middle-round priority")
        elif round_number <= 10:
            if position in ["TE", "QB"] and not qb_need_met:
                urgency += 15
        ...
        if qb_need_met and urgency > 0:
            ...
            # "no urgency" alike.
            urgency *= 0.5
            reasons.append("QB need already filled - only elite value counts now")
